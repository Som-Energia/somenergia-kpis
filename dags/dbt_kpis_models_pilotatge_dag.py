...
__doc__ = """
# Executes dbt kpis Pilotatge models
"""
...
with DAG(
    dag_id="dag_kpis_pilotatge_dbt",
    start_date=datetime(2024, 4, 28),
    schedule_interval="30 22 * * *",
    catchup=False,
    tags=["project:kpis", "dbt", "pilotatge"],
    max_active_runs=1,
    default_args=args,
    doc_md=__doc__,
) as dag:
    ...
    environment = {
        ...
        "DBT_PROFILES_DIR": "/repos/somenergia-kpis/dbt_kpis/config",
        "DBT_PROJECT_DIR": "/repos/somenergia-kpis/dbt_kpis",
    }

    dbt_transformation_task = DockerOperator(
        api_version="auto",
        task_id="dbt_build_task__kpis_pilotatge",
        ...
        command="dbt run --target=prod --selector pilotatge",
