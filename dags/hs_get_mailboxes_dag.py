...
with DAG(dag_id='hs_get_mailboxes_dag', start_date=datetime(2022,8, 17), schedule_interval='@weekly', catchup=True, tags=["Helpscout", "Extract"], default_args=args) as dag:

    task_branch_pull_ssh = build_branch_pull_ssh_task(dag=dag, task_name='hs_get_mailboxes')
    ...
    get_tags_task = DockerOperator(
        ...
        command='python3 -m datasources.helpscout.hs_get_mailboxes "{{ data_interval_start }}" "{{ data_interval_end }}" \
