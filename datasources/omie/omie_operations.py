...
from common.utils import dateCETstr_to_tzdt, graveyard_files, list_files
    ...
    for i, file_name in enumerate(hist_files["Nombre"]):
        ...
        if pathfile[-1:] == "1":
            ...
            if dry_run:
                ...
                try:
                    df.to_sql(
                        "omie_price_hour", engine, index=False, if_exists="append"
                    )
                except:
                    if verbose > 0:
...
def update_latest_hour_price(engine, verbose=2, dry_run=False):
    ...
    if pathfile[-1:] == "1":
        ...
        if dry_run:
            ...
            try:
                df.to_sql(
                    "omie_latest_price_hour", engine, index=False, if_exists="replace"
                )
            except:
                if verbose > 0:
                    print("error on insert")
...
def update_historical_hour_price(engine, verbose=2, dry_run=False):
    ...
    if pathfile[-1:] == "1":
        ...
        try:
            last_day_on_db = pd.read_sql(
                "select date from omie_price_hour order by date desc limit 1",
                con=engine,
            )["date"][0]
        except:
            if verbose > 0:
        ...
        if last_day_on_db >= last_day_new_request:
            if verbose > 1:
                print(
                    f"Last record on db is older than requested ({last_day_on_db} >= {last_day_new_request}). Skipping."
                )
            return 1
...
def update_one_energy_buy(
    engine, pdbcfile: Path, create_time, verbose=2, dry_run=False
):
    ...
    if version == ".1":
        ...
        if dry_run:
            ...
            try:
                df.to_sql(
                    name="omie_energy_buy", con=engine, index=False, if_exists="append"
                )
            except:
                if verbose > 0:
                    print("error on insert")
...
def update_energy_buy(engine, omie_pdbc_temp_dir, verbose=2, dry_run=False):
    ...
    results = {
        f: update_one_energy_buy(engine, f, create_time, verbose, dry_run)
        for f in pdbcfiles
    }
...
def get_historical_energy_buy(engine, omie_pdbc_dir: Path, verbose=2, dry_run=False):
    ...
    results = [
        update_one_energy_buy(
            engine, f, create_time=create_time, verbose=verbose, dry_run=dry_run
        )
        for f in pdbcfiles
    ]
