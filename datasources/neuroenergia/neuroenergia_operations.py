...
def shape_neuroenergia(neuro_df, request_time_dt, create_time_dt, verbose=2):
    ...
    df_energy_forecast = (
        neuro_df.iloc[2:, : len(noms_columnes)]
...
def update_one_neuroenergia(engine, neurofile, create_time, verbose=2, dry_run=False):
    ...
    if dry_run:
        ...
        if verbose > 2:
            print(df_energy_forecast)
        try:
            df_energy_forecast.to_sql(
                "energy_buy_forecast", engine, index=False, if_exists="append"
            )
        except:
            # TODO handle exceptions
            raise
...
def update_neuroenergia(engine, neuro_dir, verbose=2, dry_run=False):
    ...
    results = {
        neurofile: update_one_neuroenergia(
            engine, neurofile, create_time, verbose, dry_run
        )
        for neurofile in neurofiles
    }
