...
from common.utils import dateCETstr_to_tzdt, restore_files
from datasources.omie.omie_operations import (
    get_historical_energy_buy,
    update_energy_buy,
    update_historical_hour_price,
)
from dbconfig import test_db, test_directories
...
class MainIntegrationTest(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        with self.engine.connect() as conn:
            conn.execute("DROP TABLE IF EXISTS omie_energy_buy")
            conn.execute("DROP TABLE IF EXISTS omie_price_hour")
            conn.execute("DROP TABLE IF EXISTS meff_precios_cierre_dia")
            conn.execute("DROP TABLE IF EXISTS energy_buy_forecast")
    ...
    def create_datasources_tables(self):
        ...
        meff_df = pd.read_csv(
            "testdata/inputdata/meff_precios_cierre_dia.csv",
            parse_dates=["dia", "request_time"],
        )
        meff_df["dia"] = meff_df["dia"].dt.date
        meff_df.to_sql(
            "meff_precios_cierre_dia", con=self.engine, if_exists="replace", index=False
        )

        energy_buy_forecast_df = pd.read_csv(
            "testdata/inputdata/energy_buy_forecast.csv",
            parse_dates=["date", "request_time"],
        )
        energy_buy_forecast_df.to_sql(
            "energy_buy_forecast", con=self.engine, if_exists="replace", index=False
        )

        omie_energy_buy_df = pd.read_csv(
            "testdata/inputdata/omie_energy_buy.csv",
            parse_dates=["date", "request_time"],
        )
        omie_energy_buy_df.to_sql(
            "omie_energy_buy", con=self.engine, if_exists="replace", index=False
        )

        omie_price_hour_df = pd.read_csv(
            "testdata/inputdata/omie_price_hour.csv",
            parse_dates=["date", "request_time"],
        )
        omie_price_hour_df["date"] = pd.to_datetime(
            omie_price_hour_df["date"], utc=True
        ).dt.tz_convert("Europe/Madrid")
        omie_price_hour_df.to_sql(
            "omie_price_hour", con=self.engine, if_exists="replace", index=False
        )
